...
public class MenuPlannerServiceTests
{
    ...
    {
        _service = new MenuPlannerService(_menuPlanRepository, _residentRepository, _topicEventSender.Object);
    }
    ...
    [Fact]
    public async Task AttendMenuPlan_should_send_MenuPlanUpdated_event()
    {
        // Arrange
        var menuPlan = new MenuPlan
        {
        ...
        MenuPlanUpdatedEventMessage? sentMessage = null;
        AddMessageTopicCallback<MenuPlanUpdatedEventMessage>(MenuPlanUpdatedEventMessage.Topic,
            msg => sentMessage = msg);
        ...
        sentMessage.UpdatedMenuPlan.Id.ShouldBe(menuPlan.Id);
        sentMessage.UpdatedMenuPlan.ParticipantIds.ShouldContain(resident.Id);
    }
    ...
    [Fact]
    public async Task UnattendMenuPlan_should_send_MenuPlanUpdated_event()
    {
        // Arrange
        var resident = GenerateSampleResident(Guid.NewGuid());
        ...
        sentMessage.ShouldNotBeNull();
        sentMessage.UpdatedMenuPlan.Id.ShouldBe(menuPlan.Id);
        sentMessage.UpdatedMenuPlan.ParticipantIds.ShouldNotContain(resident.Id);
    }
    ...
    [Fact]
    public async Task SignUpForCooking_should_send_MenuPlanChanged_event()
    {
        // Arrange
        var resident = GenerateSampleResident(Guid.NewGuid());
        ...
        sentMessage.ShouldNotBeNull();
        sentMessage.UpdatedMenuPlan.Id.ShouldBe(menuPlan.Id);
        sentMessage.UpdatedMenuPlan.ChefIds.ShouldNotBeNull().ShouldContain(resident.Id);
    }
    ...
    [Fact]
    public async Task RemoveFromCooking_should_send_MenuPlanChanged_event()
    {
        // Arrange
        var resident = GenerateSampleResident(Guid.NewGuid());
        ...
        sentMessage.ShouldNotBeNull();
        sentMessage.UpdatedMenuPlan.Id.ShouldBe(menuPlan.Id);
        sentMessage.UpdatedMenuPlan.ChefIds.ShouldNotBeNull().ShouldNotContain(resident.Id);
    }

    private static Resident GenerateSampleResident(Guid id, List<Guid>? participatesInIds = null)
    {
        return new Resident
        {
            Id = id,
            ParticipatesInIds = participatesInIds ?? new List<Guid>(),
            Username = "testuser",
    ...
    private void AddMessageTopicCallback<TMessage>(string topicName, Action<TMessage> callback)
    {
        _topicEventSender.Setup(x => x.SendAsync(topicName,
                It.IsAny<TMessage>(), It.IsAny<CancellationToken>()))
            .Callback<string, TMessage, CancellationToken>((_, msg, _) => { callback(msg); });
    }
}